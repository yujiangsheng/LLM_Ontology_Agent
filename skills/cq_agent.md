...
3. 明确、无歧义
4. 确保生成的 CQ 包含不同类型的答案（list, boolean, count, description），并尽量保持分布均衡。
...
5. **答案类型采样控制**: 为保证 CQ 多样性，需在生成过程中对每种 answer_type 进行采样控制，确保至少有 3 个不同类型的 CQ。
...
| CQ 多样性 | 0.3 | 不同 expected_answer_type 的分布均衡度 |
| CQ 明确性 | 0.1 | 不含模糊/重复/过于笼统的 CQ |
...
| 2026-04-08 | ?.?.? | prompt_rewrite: 在提示词中加入对 CQ 类型分布的要求：'请确保生成的 CQ 包含不同类型的答案 | 自动诊断 | 待验证 |
| 2026-04-08 | ?.?.? | strategy_add: 增加策略：'为保证 CQ 多样性，需在生成过程中对每种 answer_type  | 自动诊断 | 待验证 |
| 2026-04-08 | ?.?.? | parameter_tune: 将 CQ 多样性指标的权重提升至 0.3，以更强调其重要性。 | 自动诊断 | 待验证 |
